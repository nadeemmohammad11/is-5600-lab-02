...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const deleteButton = document.querySelector('#btnDelete'); // fixed: added '#'
    const saveButton = document.querySelector('#btnSave');     // fixed: added '#'

    generateUserList(userData, stocksData);

    deleteButton.addEventListener('click', (event) => {
        event.preventDefault(); // fixed: prevent page reload
        ...
        if (userIndex !== -1) {
            ...
            // Optional: clear form and portfolio after delete
    ...
    saveButton.addEventListener('click', (event) => {
        event.preventDefault(); // fixed: changed from e to event
